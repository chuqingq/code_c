...
class ServiceDiscover {
 ...
 private:
  ...
  friend inline void discover_buf_alloc(uv_handle_t *tcp, size_t size,
                                        uv_buf_t *buf);
  friend inline void discover_read_cb(uv_udp_t *udp, ssize_t nread,
                                      const uv_buf_t *buf,
                                      const struct sockaddr *addr,
                                      unsigned flags);
};

inline void discover_buf_alloc(uv_handle_t *tcp, size_t size, uv_buf_t *buf) {
  auto discover = (ServiceDiscover *)tcp->data;
  buf->base = discover->buffer_;
  buf->len = sizeof(discover->buffer_);
}

inline void discover_read_cb(uv_udp_t *udp, ssize_t nread, const uv_buf_t *buf,
...
ServiceDiscover::ServiceDiscover(uv_loop_t *loop) {
  ...
  r = uv_udp_recv_start(&udp_server_, discover_buf_alloc, discover_read_cb);
  if (r < 0) {
...
inline void discover_close_cb(uv_handle_t *handle) {}

void ServiceDiscover::Stop() {
  uv_close((uv_handle_t *)&udp_server_, discover_close_cb);
  uv_close((uv_handle_t *)&udp_client_, discover_close_cb);
}