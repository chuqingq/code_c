// gcc -g -o producer -DPRODUCER shmblock.c -pthread -Wall; gcc -g -o consumer shmblock.c -pthread -Wall
// ./producer &; ./consumer
// 改为pthread实现
#include <stdint.h>
#include <stdlib.h>
...
// 生产者申请第一个能够写入的entry
uint8_t *AcquireBlockToWrite(void *s)
{
    spmc_t *spmc = (spmc_t *)s;
...
// 生产者写入后释放写锁
void ReleaseWrittenBlock(spmc_t *spmc, const uint8_t *buf)
{
    // 计算entry的位置
...
// 对申请的entry的buf只读
uint8_t *AcquireBlockToRead(void *s)
{
    spmc_t *spmc = (spmc_t *)s;
...
// 读取完成后解锁
void ReleaseReadBlock(spmc_t *spmc, const uint8_t *buf)
{
    int i = (buf - spmc->buf) / spmc->buf_size;
...
void *produce(void *not_used)
{
    ...
    {
        uint8_t *buf = AcquireBlockToWrite(spmc);
        if (buf == NULL)
        {
...
void *consume(void *not_used)
{
    ...
    for (;;)
    {
        uint8_t *buf = AcquireBlockToRead(spmc);
        if (buf == NULL)
        {
