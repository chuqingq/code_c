// g++ -g -lrt -o producer -DPRODUCER shmblock3.cpp -pthread -Wall; g++ -g -lrt -o consumer shmblock3.cpp -pthread -Wall
// ./producer &; ./consumer
...
public:
    ...
    int LockForRead() {
        while (1) { // TODO 未考虑重试次数，一直读，直到读到有效内容
            int i = read_pos_.load(std::memory_order_relaxed); // 这里使用内存排序，提升明显
            if (i < 0) {
              // 未曾写入过
