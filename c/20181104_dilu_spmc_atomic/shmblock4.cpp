...
class State {
 ...
 private:
  ...
  int write_pos_{
      0};  // producer下次将要写入的位置。只有producer访问，当前是单生产者模式，无需原子
  alignas(64) std::atomic<int> read_pos_{
      -1};  // producer最近写入成功的位置，即消费者需要读取的位置。是producer和consumer同步的关键
...
bool ShmBlock::OpenOrCreate() {
  ...
  if (ftruncate(fd, cap_) <
      0) {  // TODO cap_应该时off_t类型，64位是long long int
